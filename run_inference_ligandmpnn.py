...
from LASErMPNN.utils.model_ligandmpnn import LigandMPNN, Sampled_Output
from LASErMPNN.utils.pdb_dataset_ligandmpnn import BatchData, UnprocessedLigandData, idealize_backbone_coords
    ...
    def output_batch_data(self, fix_beta: bool = False, num_copies: int = 1) -> BatchData:
        ...
        if num_copies == 1:
            batch_data_dict = {
                ...
                'sidechain_in_hull_mask': zeros_bool,
            }
        else:
            batch_data_dict = {
                ...
                'sidechain_in_hull_mask': torch.cat([zeros_bool for _ in range(num_copies)], dim=0),
            }
...
def load_model_from_parameter_dict(path_to_weights: str, inference_device: Union[str, torch.device], strict: bool = True) -> Tuple[LigandMPNN, dict]:
    """
    ...
    # Load the model
    model = LigandMPNN(**training_parameter_dict['model_params']).to(inference_device)
...
@torch.no_grad()
def sample_model(
    model: LigandMPNN, batch_data: BatchData, sequence_temp: Optional[float], bb_noise: float, params: dict, use_edo: bool = False, chi_temp: Optional[float] = None,
    ...
    if not use_edo:
        ...
        sampling_output = model.sample(
            ...
            disable_pbar=disable_pbar, seq_min_p=seq_min_p,
...
def output_ligand_structure(ligand_info: LigandInfo) -> pr.AtomGroup:
    ...
    ligand = pr.AtomGroup('LigandMPNN Generated Ligand')
...
def run_inference(
    ...
    full_atom_coords = model.rotamer_builder.build_rotamers(batch_data.backbone_coords, torch.zeros(batch_data.backbone_coords.shape[0], 4, device=batch_data.device), sampled_output.sampled_sequence_indices, add_nonrotatable_hydrogens=True)
