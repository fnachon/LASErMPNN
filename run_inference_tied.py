...
@torch.no_grad()
def sample_model(
    model: LASErMPNN, batch_data1: BatchData, batch_data2: BatchData, interpolation_lambda: float, sequence_temp: Optional[float], bb_noise: float, params: dict, use_edo: bool = False, chi_temp: Optional[float] = None,
    verbose: bool = False, disable_pbar: bool = False, fs_sequence_temp: Optional[float] = None, 
    ignore_chain_mask_zeros: bool = False, disabled_residues: Optional[List[str]] = ['X', 'C'], repack_all: bool = False,
    ...
    batch_data1.to_device(model.device)
    batch_data1.construct_graphs(
    ...
    batch_data2.to_device(model.device)
    batch_data2.construct_graphs(
        model.rotamer_builder,
        model.ligand_featurizer, 
        **params['model_params']['graph_structure'],
        protein_training_noise = bb_noise,
    ...
    batch_data1.generate_decoding_order(stack_tensors=True)
    batch_data2.decoding_order = batch_data1.decoding_order
    ...
    if fs_sequence_temp is not None:
        ...
        sample_temperature_vector = torch.full((batch_data1.num_residues,), sequence_temp, dtype=torch.float, device=model.device)
        sample_temperature_vector[batch_data1.first_shell_ligand_contact_mask] = fs_sequence_temp

    sampling_output_1, sampling_output_2 = model.tied_sample(
        batch_data1, batch_data2, 
        lambda_=interpolation_lambda,
        ...
        chi_angle_sample_temperature=chi_temp, disable_pbar=disable_pbar,
        # ignore_chain_mask_zeros=ignore_chain_mask_zeros, 
        disabled_residues=disabled_residues, repack_all=repack_all,
        # disable_charged_residue_mask=None if not disable_charged_fs else batch_data1.first_shell_ligand_contact_mask
    )

    
    return sampling_output_1
...
def run_inference(
    input_pdb_path_1: str, input_pdb_path_2: str, interpolation_lambda: float, path_to_weights: str, sequence_temp: Optional[float], bb_noise: float, inference_device: str, fix_beta: bool, output_path: str, strict_load: bool, use_edo: bool, 
    ...
    disable_charged_fs: bool = False, constrain_ala_gly_sampling_to_exposed_non_secondary_structure: bool = False,
    budget_residue_sele_string: Optional[str] = None, designs_per_input: int = 1, ala_budget: Optional[int] = 4, gly_budget: Optional[int] = 0
):
    for design_idx in tqdm(range(designs_per_input), desc="Running designs", disable=designs_per_input == 1):
        print("Loading", input_pdb_path_1, "and", input_pdb_path_2, "and running inference with", path_to_weights, "on", inference_device)
        if bb_noise > 0:
            print(f"Noising backbone with {bb_noise}A Gaussian noise.")

        # Parse the input PDB file.
        protein_hv1 = get_protein_hierview(input_pdb_path_1)
        protein_hv2 = get_protein_hierview(input_pdb_path_2)

        if fix_beta:
            unique_betas = np.unique(np.concatenate([x.getBetas() for x in protein_hv1.iterChains()])) # type: ignore
        ...
        data = ProteinComplexData(
            protein_hv1, input_pdb_path_1, treat_noncanonical_as_ligand=noncanonical_aa_ligand, 
            first_shell_ca_distance=fs_calc_ca_distance, first_shell_buried_only=(not fs_calc_no_calc_burial), first_shell_burial_calc_hull_alpha=fs_calc_burial_hull_alpha_value
        )
        batch_data = data.output_batch_data(fix_beta)

        data2 = ProteinComplexData(
            protein_hv2, input_pdb_path_2, treat_noncanonical_as_ligand=noncanonical_aa_ligand,
            first_shell_ca_distance=fs_calc_ca_distance, first_shell_buried_only=(not fs_calc_no_calc_burial), first_shell_burial_calc_hull_alpha=fs_calc_burial_hull_alpha_value
        )
        batch_data2 = data2.output_batch_data(fix_beta)
        ...
        if ignore_ligand:
            ...
            batch_data2.unprocessed_ligand_input_data.lig_coords = torch.empty(0, 3)
            batch_data2.unprocessed_ligand_input_data.lig_batch_indices = torch.empty(0, dtype=torch.long)
            batch_data2.unprocessed_ligand_input_data.lig_subbatch_indices = torch.empty(0, dtype=torch.long)
            batch_data2.unprocessed_ligand_input_data.lig_burial_maskmask = torch.empty(0, dtype=torch.bool)
            batch_data2.unprocessed_ligand_input_data.lig_atomic_numbers = torch.empty(0, dtype=torch.long)
        ...
        if repack_only:
            ...
            batch_data2.chain_mask = torch.ones_like(batch_data2.chain_mask)
        else:
            print(f"Tied inference. Designing {batch_data.num_residues - batch_data.chain_mask.sum()} Residues. Fixing {batch_data.chain_mask.sum()} Residues.")
        ...
        if budget_string_is_set or constrain_ala_gly_sampling_to_exposed_non_secondary_structure:
            ...
            if not budget_string_is_set:
                budget_residue_sele_string = compute_constrained_ala_gly_residues(protein_hv1.getAtoms())

            # Compute the budget residue mask.
            reference_mask_res_indices = protein_hv1.getAtoms().select(f"name CA").getResindices()
            mask_sele_indices = protein_hv1.getAtoms().select(f"(same residue as ({budget_residue_sele_string})) and name CA").getResindices()
        ...
        sampled_output = sample_model(
            model, batch_data, batch_data2, interpolation_lambda, sequence_temp, bb_noise, params, use_edo, fs_sequence_temp=fs_sequence_temp, repack_all=repack_only, budget_residue_mask=budget_residue_mask,
            disable_charged_fs=disable_charged_fs, ala_budget=ala_budget, gly_budget=gly_budget
        ) 
        ...
        output_path_ = (output_path_.parent / f'{output_path_.stem}_{design_idx}{output_path_.suffix}') if designs_per_input > 1 else output_path_
        pr.writePDB(str(output_path_), out_prot)
        print("Wrote laser designed output to", output_path_.absolute())
...
def parse_args():
    ...
    # Required input:
    parser.add_argument('input_pdb_code_1', type=str, help='Path to the input PDB file.')
    parser.add_argument('input_pdb_code_2', type=str, help='Path to the input PDB file.')
    parser.add_argument('--tied_probability_interpolation_lambda', '-i', dest='tied_probability_interpolation_lambda', type=float, default=0.0, help='Lambda value for interpolating between the two model output distributions during tied decoding. Final probabilities are computed as: lambda * prob1 + (1-lambda) * prob2. Default is 0.0 which corresponds to using only the first input structure\'s model output distribution.')
    parser.add_argument('--num_designs', dest='designs_per_input', type=int, default=1, help='Number of sequences to design per input structure. Default is 1.')
    ...
    parser.add_argument('--disabled_residues', dest='disabled_residues', type=str, default='X,C', help='Comma separated list of one letter amino acid codes to disable sampling of. Default is "X,C" which corresponds to any residues with unknown amino acids and Cysteines.')
    # Flags:
    ...
    parser.add_argument('--budget_residue_sele_string', type=str, default='', help='A ProDy selection string to specify residues to constrain sampling for. If set, only residues that satisfy the selection criteria will be constrained. Ex: "same residue as (chain A and resnum 50)". See ProDy documentation for selection string syntax: http://prody.csb.pitt.edu/manual/reference/proteins/selection.html')
    ...
    parser.add_argument('--ala_budget', type=int, default=4, help='Max number of alanine residues that can be sampled in exposed non-secondary structured residues when --constrain_ala_gly_sampling_to_exposed_non_secondary_structure is set.')
    parser.add_argument('--gly_budget', type=int, default=0, help='Max number of glycine residues that can be sampled in exposed non-secondary structured residues when --constrain_ala_gly_sampling_to_exposed_non_secondary_structure is set.')
...
if __name__ == "__main__":
    ...
    run_inference(
        parsed_args.input_pdb_code_1, parsed_args.input_pdb_code_2, parsed_args.tied_probability_interpolation_lambda,
        parsed_args.model_weights, sequence_temp, backbone_noise, 
        ...
        budget_residue_sele_string=parsed_args.budget_residue_sele_string,
        constrain_ala_gly_sampling_to_exposed_non_secondary_structure=parsed_args.constrain_ala_gly_sampling_to_exposed_non_secondary_structure,
        designs_per_input=parsed_args.designs_per_input,
        ala_budget=parsed_args.ala_budget, gly_budget=parsed_args.gly_budget
    )
