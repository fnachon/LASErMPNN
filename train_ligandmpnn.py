...
if __name__ == '__main__':
    ...
    VISIBLE_DEVICES = ['cuda:4', 'cuda:5', 'cuda:6', 'cuda:7']
...
from LASErMPNN.utils.model_ligandmpnn import LigandMPNN
...
from LASErMPNN.utils.pdb_dataset_ligandmpnn import UnclusteredProteinChainDataset, LigandMPNNDatasetSampler, collate_sampler_data, BatchData, invert_dict, chain_list_to_protein_chain_dict
...
def save_model_checkpoint(model, params, optimizer, optimizer_steps, epoch_num) -> None:
    """
...
def train(rank, world_size, params):
    ...
    collate_fn = lambda x: collate_sampler_data(x, params['use_xtal_additive_ligands'], recompute_all_cb_atoms=params['recompute_all_cb_atoms'])
                ...
                for batch_idx, batch in enumerate(train_dataloader):
                    ...
                    batch.construct_graphs(
                        ...
                        **params['model_params']['graph_structure'], 
                        ...
                        build_hydrogens=params['model_params']['build_hydrogens'],
                        use_aliphatic_ligand_hydrogens=params['use_aliphatic_ligand_hydrogens'],
                        atomization_fraction=params['atomization_fraction'],
                    )
                    batch.generate_decoding_order()
                    ...
                    with autocast(device_type='cuda', dtype=torch.float16):
                        ...
                        loss = world_size * (seq_loss + msa_loss)
                    ...
                    if steps_taken % params['log_interval'] == 0:
                        ...
                        if rank == 0:
                            ...
                            if (steps_taken % (params['train_logs_per_test_epoch'] * params['log_interval']) == 0) or not initial_log:
                                ...
                                save_model_checkpoint(model, params, optimizer, steps_taken, epoch)
...
if __name__ == "__main__":
    ...
    params = {
        ...
        'use_per_atom_backbone_noise': True,

        'devices': VISIBLE_DEVICES,

        'checkpoint_to_resume_from': None,
        'output_weights_checkpoint_prefix': CURR_FILE_DIR_PATH / 'model_weights/training_checkpoint_lmpnn',
        ...
        'atomization_fraction': 0.02,
        'protein_training_noise': 0.1 if augment else 0.0,
        'ligand_training_noise': 0.1 if augment else 0.0,
        'subgraph_only_dropout_rate': -1 if augment else 0.0,

        'sample_randomly': True,
        'autoregressive_test_batch_sample_fraction': 0.5 if not debug else 0.25,
        'model_params': {
