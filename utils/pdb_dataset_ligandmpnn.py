...
from .burial_calc import compute_fast_alphahull_burial_mask, compute_fast_ligand_burial_mask
...
class UnprocessedLigandData():
    ...
    lig_burial_mask: Optional[torch.Tensor] = None
    ...
    def extend(self, other: 'UnprocessedLigandData') -> 'UnprocessedLigandData':
        ...
        new_burial_mask = None
        if self.lig_burial_mask is not None and other.lig_burial_mask is not None:
            new_burial_mask = torch.cat([self.lig_burial_mask, other.lig_burial_mask], dim=0)
        elif self.lig_burial_mask is not None and other.lig_burial_mask is None:
            new_burial_mask = torch.cat([self.lig_burial_mask, torch.zeros_like(other.lig_subbatch_indices, dtype=torch.bool)], dim=0)
        elif self.lig_burial_mask is None and other.lig_burial_mask is not None:
            new_burial_mask = torch.cat([torch.zeros_like(self.lig_subbatch_indices, dtype=torch.bool), other.lig_burial_mask], dim=0)
        ...
        return UnprocessedLigandData(
            ...
            lig_burial_mask=new_burial_mask
        )
    
    def set_burial_mask(self, burial_mask: torch.Tensor, batch_idx: int) -> None:
...
class LigandData():
    ...
    ligand_burial_mask: Optional[torch.Tensor] = None
...
class BatchData():
    ...
    sidechain_in_hull_mask: torch.Tensor
    ...
    def construct_graphs(
            self, rotamer_builder: RotamerBuilder, ligand_featurizer: LigandFeaturizer,
            pr_pr_knn_graph_k: int, lig_pr_distance_cutoff: float, lig_pr_knn_graph_k: int, 
            protein_training_noise: float, ligand_training_noise: float, 
            build_hydrogens: bool, use_aliphatic_ligand_hydrogens: bool = True, atomization_fraction: float = 0.00
    ):
        """
        ...
        all_lig_coords, all_prot_prot_eidces, all_lig_prot_eidces, all_lig_lig_eidces, all_lig_nodes, all_lig_batch_indices, all_lig_subbatch_indices, all_lig_node_masks, first_shell_masks, all_lig_atomic_numbers, all_lig_burial_masks = [], [], [], [], [], [], [], [], [], [], []
        ...
        for batch_idx in range(self.batch_indices.max() + 1):
            ...
            curr_complex_fa_coords = rotamer_builder.build_rotamers(curr_complex_backbone_coords, self.chi_angles[curr_batch_prot_mask], self.sequence_indices[curr_batch_prot_mask], add_nonrotatable_hydrogens=build_hydrogens).float()

            # Identify which residues to atomize.
            n_frac = int(atomization_fraction * curr_complex_fa_coords.shape[0])
            indices_to_atomize = torch.randperm(curr_complex_backbone_coords.shape[0], device=self.device)[:n_frac]
            pseudoligand_coords = curr_complex_fa_coords[indices_to_atomize]
            curr_nan_mask = ~pseudoligand_coords.isnan().any(dim=-1)
            pseudoligand_coords = pseudoligand_coords[curr_nan_mask]
            pseudoligand_nodes, pseudoligand_batch_indices, pseudoligand_subbatch_indices, pseudoligand_atomic_numbers = ligand_featurizer.masked_generate_ligand_nodes_from_amino_acid_labels(self.sequence_indices[curr_batch_prot_mask][indices_to_atomize], self.batch_indices[curr_batch_prot_mask][indices_to_atomize], torch.arange(n_frac, device=self.device), curr_nan_mask)
            assert pseudoligand_coords.shape[0] == pseudoligand_nodes.shape[0], f"Mismatched shapes for pseudoligand nodes and coordinates. {pseudoligand_nodes.shape=}, {pseudoligand_coords.shape=}"

            # Mark atomized indices for dropping from protein graph.
            residues_to_drop_mask[indices_to_atomize] = True

            # Protein-Only Graph Generation.
            if (real_ligand_node_mask.numel() == 0) or ((not real_ligand_node_mask.any().item())):
            ...
            if real_ligand_node_mask.any().item():
                ...
                if self.unprocessed_ligand_input_data.lig_burial_mask is not None:
                    curr_lig_burial_masks = self.unprocessed_ligand_input_data.lig_burial_mask[real_ligand_node_mask]
                else:
                    curr_lig_burial_masks = torch.zeros_like(curr_real_ligand_mask, dtype=torch.bool)
            
            # Merge the sampled pseudoligands with the real ligands.
            if n_frac > 0:
                ligand_is_pseudoligand_mask = torch.zeros(pseudoligand_coords.shape[0], dtype=torch.bool, device=self.device)
                pseudoligand_burial_mask = compute_fast_ligand_burial_mask(curr_complex_backbone_coords[~residues_to_drop_mask][:, 1].cpu().numpy(), pseudoligand_coords.cpu().numpy(), num_rays=5).to(self.device)

                if real_ligand_node_mask.any().item():
                    assert not any([x is None for x in (curr_lig_coords, curr_lig_nodes, curr_batch_idces, curr_subbatch_idces, curr_lig_atomic_numbers, curr_real_ligand_mask)]), "Unreachable."
                    curr_lig_coords = torch.cat([curr_lig_coords, pseudoligand_coords], dim=0)
                    curr_lig_nodes = torch.cat([curr_lig_nodes, pseudoligand_nodes], dim=0)
                    curr_lig_atomic_numbers = torch.cat([curr_lig_atomic_numbers, pseudoligand_atomic_numbers], dim=0)
                    curr_batch_idces = torch.cat([curr_batch_idces, pseudoligand_batch_indices], dim=0)
                    curr_subbatch_idces = torch.cat([curr_subbatch_idces, pseudoligand_batch_indices + curr_subbatch_idces.max() + 1], dim=0)
                    curr_real_ligand_mask = torch.cat([curr_real_ligand_mask, ligand_is_pseudoligand_mask], dim=0)
                    curr_lig_burial_masks = torch.cat([curr_lig_burial_masks, pseudoligand_burial_mask], dim=0)
                else:
                    curr_lig_coords, curr_lig_nodes, curr_lig_atomic_numbers, curr_batch_idces, curr_subbatch_idces = pseudoligand_coords, pseudoligand_nodes, pseudoligand_atomic_numbers, pseudoligand_batch_indices, pseudoligand_subbatch_indices
                    curr_real_ligand_mask = ligand_is_pseudoligand_mask
                    curr_lig_burial_masks = pseudoligand_burial_mask

            # Use available ligand data to construct the graphs.
            if all([x is not None for x in (curr_lig_coords, curr_lig_nodes, curr_batch_idces, curr_subbatch_idces, curr_lig_atomic_numbers, curr_real_ligand_mask)]):
                ...
                curr_lig_lig_eidx = radius_graph(curr_lig_coords, r=20.0, loop=True, batch=curr_subbatch_idces, max_num_neighbors=256)
                ...
                curr_fa_coords = curr_complex_fa_coords[~residues_to_drop_mask]
                ...
                all_lig_burial_masks.append(curr_lig_burial_masks)
        ...
        all_lig_burial_masks = torch.cat(all_lig_burial_masks, dim=0) if len(all_lig_burial_masks) > 0 else torch.empty((0,), dtype=torch.bool, device=self.device)
        ...
        self.backbone_coords = self.backbone_coords + (protein_training_noise * torch.randn_like(self.backbone_coords))
        ...
        self.ligand_data = LigandData(
            ...
            all_lig_node_masks, all_lig_atomic_numbers, all_lig_burial_masks
        )
    ...
    def _apply_mask_to_residue_metadata_tensors(self, mask: torch.Tensor) -> None:
        ...
        list_of_residue_metadata_tensor_keys = [
            ...
            'first_shell_ligand_contact_mask', 'sc_mediated_hbond_counts', 'sidechain_in_hull_mask',
        ]
        for k,v in self.__dict__.items():
...
def collate_sampler_data(data: list, use_xtal_additive_ligands: bool, disable_convex_hull: bool = False, recompute_all_cb_atoms: bool = True) -> BatchData:
    """
    ...
    for batch_idx, (complex_data, chain_key) in enumerate(data):
        ...
        if all_ligand_data is not None:
            if not disable_convex_hull:
                all_ligand_coords = all_ligand_data.lig_coords[all_ligand_data.lig_batch_indices == batch_idx]
                burial_mask, lig_burial_mask = compute_fast_alphahull_burial_mask(all_backbone_coords.numpy(), all_ligand_coords.numpy(), num_rays=3)
                all_ligand_data.set_burial_mask(lig_burial_mask, batch_idx)
            else:
                burial_mask = torch.zeros(all_backbone_coords.shape[0], dtype=torch.bool)
                lig_burial_mask = torch.zeros(all_ligand_data.lig_coords.shape[0], dtype=torch.bool)
        else:
            if not disable_convex_hull:
                burial_mask, _ = compute_fast_alphahull_burial_mask(all_backbone_coords.numpy(), None, num_rays=3)
            else:
                burial_mask = torch.zeros(all_backbone_coords.shape[0], dtype=torch.bool)
        all_batch_data['backbone_coords'].append(all_backbone_coords)
        all_batch_data['sidechain_in_hull_mask'].append(burial_mask)
...
class UnclusteredProteinChainDataset(Dataset):
    ...
    def __init__(self, params):
        ...
        if not os.path.exists(str(metadata_shelve_path) + '.dat'):
...
class ClusteredDatasetSampler(Sampler):
    ...
    def __init__(
        ...
        self.cluster_to_chains = self.filter_clusters(is_test_dataset_sampler, single_protein_debug)
    ...
    def sample_clusters(self) -> None:
        ...
        for cluster, chains in self.cluster_to_chains.items():
            # With some fraction sample only from proteins with ligands for a given cluster if possible.
            chains_ = []
            if torch.rand((1,), generator=self.generator).item() < self.bias_sample_to_proteins_with_ligand_fraction:
                chains_ = [x for x in chains if self.dataset.index_to_num_ligand_contacting_residues[self.dataset.chain_key_to_index[x]] >= self.min_num_ligand_contacting_residues_for_bias_sample]

            # Handle no chains contacting ligands in the cluster or the bias is not applied.
            if len(chains_) == 0:
                chains_ = chains
            ...
            pdb_to_assembly_chains_map = chain_list_to_protein_chain_dict(chains_)
