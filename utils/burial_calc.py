...
from .pdb_dataset import idealize_backbone_coords
...
def calc_residue_burial(input_path: os.PathLike, num_rays = 5):
    if not Path(input_path).exists():
        raise FileNotFoundError(f'{input_path} is missing.')

    protein = pr.parsePDB(str(input_path))
    protein.setBetas(0.0)

    frame_atoms = []
    phi_psi_angles = []
    for residue in protein.iterResidues():
        if all([x in residue.getNames() for x in ('N', 'CA', 'C')]) :
            frame_atoms.append(np.stack(
                [
                    residue.select('name N').getCoords()[0],  # N
                    residue.select('name CA').getCoords()[0], # CA
                    np.array([np.nan] * 3),                   # CB
                    residue.select('name C').getCoords()[0],  # C
                    np.array([np.nan] * 3),                   # O
                ]
            ))
            phi_psi_angles.append(calc_phi_psi(residue))

    bb_coords = torch.from_numpy(np.stack(frame_atoms))
    phi_psi_angles = torch.from_numpy(np.nan_to_num(np.stack(phi_psi_angles), 0.0))
    ideal_bb_coords = idealize_backbone_coords(bb_coords.float(), phi_psi_angles.float())

    lig_coords = protein.select('not protein').getCoords()
    ca_coords = ideal_bb_coords[:, 1]
    cb_coords = ideal_bb_coords[:, 2]

    cb_buried = compute_fast_ligand_burial_mask(ca_coords.float().numpy(), cb_coords.float().numpy(), num_rays=num_rays).numpy()
    lig_buried = compute_fast_ligand_burial_mask(ca_coords.float().numpy(), lig_coords, num_rays=num_rays).numpy()
    ...
    for residue in protein.iterResidues():
        if all([x in residue.getNames() for x in ('N', 'CA', 'C')]) :
            ...
            idx += 1
    protein.select('not protein').setBetas(lig_buried)

    return cb_buried, lig_buried, protein
